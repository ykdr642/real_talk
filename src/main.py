    ...
    def makecmd(self):
        ...
        for chousei in self.chouseiConfig:
            if str(self.tasklist[0][2]) == chousei[0]:
                if len(chousei) == 4:
                    self.cmd = 'seikasay.exe -cid ' + self.cv + ' -speed ' + chousei[1] + ' -pitch ' + chousei[2] + ' -intonation ' + chousei[3] + ' \"' +self.tasklist[0][1] + '\"'
                    chousei_is_success = True
                    break
                elif len(chousei) == 7:
                    self.cmd = 'seikasay.exe -cid ' + self.cv + ' -speed ' + chousei[1] + ' -pitch ' + chousei[2] + ' -intonation ' + chousei[3] + ' -happiness ' + chousei[4] + ' -hatred ' + chousei[5] + ' -sadness ' + chousei[6] + ' \"' + self.tasklist[0][1]+ '\"'
                    chousei_is_success = True
                    break                    
        if not chousei_is_success:
            self.cmd = 'seikasay.exe -cid ' + self.cv + ' \"' + self.tasklist[0][1] + '\"'
        ...
        #初期化
            ...
            while True:
                ...
                if needchange and vanishtimer < 150:
                    ...
                    if len(eye[face]) > 0:
                        drawmatrix = cv2.resize(drawmatrix,(eye[face][bodynum].shape[1],eye[face][bodynum].shape[0]))
                        drawmatrix *= 1 - maskE
                        drawmatrix += eye[face][bodynum][:,:,:3] * maskE

                    if len(anime_talk[face]) > 0 and speekNow:
                        drawmatrix = cv2.resize(drawmatrix,(anime_talk[face][bodynum].shape[1],anime_talk[face][bodynum].shape[0]))
                        drawmatrix *= 1 - maskA
                        drawmatrix += anime_talk[face][bodynum][:,:,:3] * maskA


                    if len(anime_wait[face]) > 0 and not speekNow:
                        drawmatrix = cv2.resize(drawmatrix,(anime_wait[face][bodynum].shape[1],anime_wait[face][bodynum].shape[0]))
                        drawmatrix *= 1 - maskA
                        drawmatrix += anime_wait[face][bodynum][:,:,:3] * maskA
...
class MyFrame(wx.Frame):
    def __init__(self, parent, title):
        ...
        label4_2 = wx.StaticText(panel4_2, wx.ID_ANY, '登録番号',style=wx.TE_RIGHT, size=(300, 50))
